...
__all__ = ['bboxIOU', 'doNMS', 'encodeBox', 'decodeAllBox']
...
def bboxIOU(bboxA, bboxB):
    ...
    width_and = (A_width + B_width) - (xmax - xmin)               # 宽的交集
    height_and = (A_height + B_height) - (ymax - ymin)            # 高的交集

    if width_and < 0.0001 or height_and < 0.0001:
        return 0

    area_and = width_and*height_and
...
def encodeBox(config, box, predBox):
    ...
    ew = (box[2] - box[0]) / pw
    eh = (box[3] - box[1]) / ph
    ew = int(math.log(ew)*5)
    eh = int(math.log(eh)*5)
